...
async function verifyPayment(reference: string, accessToken: string): Promise<VerificationResult> {
	...
	try {
		const response = await fetch(`${backEndUrl}/payments/credit/verify/${reference}`, {
			headers: {
...
export default async function VerifyCreditPaymentPage({ searchParams }: { searchParams: Promise<{ trxref?: string; reference?: string }> }) {
	...
	if (!reference) {
		redirect(configurations.urls.User.Subscription);
	}
							...
							<div className="space-y-3">
								...
								<div className="flex justify-between items-center">
									...
									<span className="font-bold text-gray-900">
										{payment.creditsPurchased} credit{payment.creditsPurchased !== 1 ? 's' : ''}
									</span>
								</div>

								<div className="flex justify-between items-center">
									<span className="text-gray-600">Type</span>
									<span className="font-medium text-gray-900">{payment.type === 'CREDIT_PURCHASE' ? 'Credit Purchase' : 'Subscription Renewal'}</span>
								</div>
						...
						{isSuccess ? (
							...
							<>
								<div className="text-center py-4">
									...
									<Link
										href={`${configurations.urls.User.VerifyPayment}?reference=${reference}`}
