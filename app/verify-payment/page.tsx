...
async function verifyPayment(reference: string): Promise<VerificationResult> {
	const { backEndUrl } = configurations.envs;

	try {
		const response = await fetch(`${backEndUrl}/payments/booking/verify/public/${reference}`, {
			headers: {
						...
						{isSuccess ? (
							...
							<>
								<div className="text-center py-4">
									...
									<Link
										href={`${configurations.urls.Public.VerifyBookingPayment}?r	eference=${reference}`}
